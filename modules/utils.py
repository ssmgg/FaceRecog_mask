...
from modules.dataset_arc import load_tfrecord_dataset
import math
...
#   Visulization                                                              #
###############################################################################
def draw_bbox_half(img, ann, img_height, img_width):
    ...
    cv2.rectangle(img,
                  (x1, y1),
                  (x2, bounding_box[1]+bounding_box[3]),
                  (0, 255, 0), 2)

    # landmark
    if ann[14] > 0:
        cv2.circle(img, (keypoints['left_eye']), 1, (255, 255, 0), 2)
        cv2.circle(img, (keypoints['right_eye']), 1, (0, 255, 255), 2)
        cv2.circle(img, (keypoints['nose']), 1, (255, 0, 0), 2)
        cv2.circle(img, (keypoints['mouth_left']), 1, (0, 100, 255), 2)
        cv2.circle(img, (keypoints['mouth_right']), 1, (255, 0, 100), 2)

    print(result)
    return result


def get_bbox(img, ann, img_height, img_width):
    result = []
