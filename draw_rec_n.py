...
def main(_argv):
    ...
    for path in paths:  # mmj
        ...
        try:
            if not os.path.exists('./data/black/img_rec_white/{}'.format(p)):
                os.makedirs('./data/black/img_rec_white/{}'.format(p))
                print('[*] Make dir ' + './data/black/img_rec_white/{}'.format(p))
        except OSError:
            print('Cannot creat directory - ./data/black/img_rec_white/{}'.format(p))
        ...
        for image in images:    # dis09_1_crop.jpg
            ...
            nose = keypoints['nose']
            cv2.rectangle(img_raw, (0, nose[1]), (img_width_raw, img_height_raw), (255, 255, 255), -1)

            # if outputs[0][14] > 0:
            #     cv2.circle(img_raw, (keypoints['left_eye']), 1, (255, 255, 0), 2)
            #     cv2.circle(img_raw, (keypoints['right_eye']), 1, (0, 255, 255), 2)
            #     cv2.circle(img_raw, (keypoints['nose']), 1, (255, 0, 0), 2)

            # cv2.imshow('', img_raw)
            # cv2.waitKey(0)
            cv2.imwrite('./data/black/img_rec_white/{}/{}'.format(p, os.path.basename(image)), img_raw)
